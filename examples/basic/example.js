    ...
    RedLocomotive({
		...
		function createElements() {
        	...
        	var testElement = engine.element.create('test', 'sprites/test.png', 300, 125, 100, 100),
				testTextElement = engine.text.create('test', 'Red Locomotive!', 48, -600, 100),
			...
			engine.every(function () {
				if (testTextElement.x < mainView.node[0].width) { testTextElement.x += 5; } else { testTextElement.x = -600; }
				if (testTextElement2.x > -600) { testTextElement2.x -= 5; } else { testTextElement2.x = mainView.node[0].width; }
			});
